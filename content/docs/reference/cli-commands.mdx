...
- **PDF** — Renders each slide as a JPEG image and combines them into a PDF using jsPDF
- **PPTX** — Renders each slide as a JPEG image and inserts them into a PowerPoint file using pptxgenjs

<Callout type="info">
Export is browser-based — no additional CLI commands are needed. The export functions use html-to-image to capture each slide at full resolution.
</Callout>
