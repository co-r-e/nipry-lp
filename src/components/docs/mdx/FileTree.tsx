...
function FolderIcon(): ReactNode {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-[var(--ink-soft)]">
...
function TreeNode({ node, depth }: { node: FileTreeNode; depth: number }): ReactNode {
  const paddingLeft = depth * 20;
  return (
    <>
      <div className="flex items-center gap-2 py-0.5" style={{ paddingLeft }}>
        {node.type === "directory" ? <FolderIcon /> : <FileIcon />}
        <span className={`text-sm ${node.highlight ? "font-medium text-[var(--accent)]" : "text-[var(--ink-soft)]"}`}>{node.name}</span>
      </div>
