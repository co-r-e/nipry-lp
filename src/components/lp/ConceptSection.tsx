import type { ReactNode } from "react";
import type { Dictionary } from "@/i18n/dictionaries";
import { FadeIn } from "./FadeIn";

interface ConceptSectionProps {
  dict: Dictionary;
}

const ICONS = [
  // Zero Context Switching - cookie/focus icon
  <svg key="0" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a10 10 0 1 0 10 10 4 4 0 0 1-5-5 4 4 0 0 1-5-5" /><path d="M8.5 8.5v.01" /><path d="M16 15.5v.01" /><path d="M12 12v.01" /><path d="M11 17v.01" /><path d="M7 14v.01" /></svg>,
  // Per-Slide Parallel Editing - link/code icon
  <svg key="1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z" /><polyline points="14 2 14 8 20 8" /><path d="m10 13-2 2 2 2" /><path d="m14 17 2-2-2-2" /></svg>,
  // AI Micro-Tuning - github/branch icon
  <svg key="2" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0 3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" /><path d="M9 18c-4.51 2-5-2-7-2" /></svg>,
];

export function ConceptSection({ dict }: ConceptSectionProps): ReactNode {
  return (
    <section className="concept">
      <div className="container">
        <div className="section-header text-center">
          <h2 className="section-title">{dict.concept.title}</h2>
          <p className="section-subtitle">{dict.concept.subtitle}</p>
        </div>
        <div className="grid grid-3">
          {dict.concept.cards.map((card, i) => (
            <FadeIn key={i} className="card concept-card">
              <div className="icon-wrapper">{ICONS[i]}</div>
              <h3>{card.title}</h3>
              <p>{card.description}</p>
            </FadeIn>
          ))}
        </div>
      </div>
    </section>
  );
}
