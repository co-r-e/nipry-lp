...
export function DocsHeader({ onSearchOpen }: Props): ReactNode {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-[var(--line)] bg-[var(--background)]/80 px-4 backdrop-blur-lg lg:px-6">
      <Link href="/docs" className="flex items-center" aria-label="Amaroad Docs home">
        <img src={assetUrl(isDark ? "/assets/amaroad-logo.svg" : "/assets/amaroad-logo-dark.svg")} alt="Amaroad" className="h-7 w-auto" />
      </Link>
      ...
      <div className="flex items-center gap-2">
        ...
        <Link href="/" className="hidden items-center gap-1 rounded-lg px-2 py-1.5 text-sm text-[var(--ink-soft)] transition hover:text-[var(--ink)] sm:flex">
          {backChevron}
          Back to site
        </Link>

        <Link href="/" className="rounded-lg p-2 text-[var(--ink-soft)] transition hover:text-[var(--ink)] sm:hidden" aria-label="Back to site">
          {backChevron}
        </Link>
      </div>
    </header>
