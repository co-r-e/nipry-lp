---
title: Agent Skills
description: 14 built-in Claude Code skills that extend DexCode with AI-powered workflows.
---
...
```bash
> デッキを設計して / design a deck / help me plan a presentation
```
...
```bash
> デッキを作って / create a new deck / scaffold deck from this brief
```
...
```bash
> 画像を生成 / generate an image for this slide / add a photo to slide 3
```
...
```bash
> 画像を編集 / edit this image / remove the background
```
...
```bash
> 図を描いて / draw a flowchart / create an architecture diagram
```
...
```bash
> グラレコ / graphic recording / visual note for this slide
```
...
```bash
> 動画にして / make a video from this deck / export as video
```
...
```bash
> 英語に翻訳 / translate to English / localize this deck to Japanese
```
...
```bash
> はみ出し修正 / fix overflow / content does not fit
```
...
```bash
> プリフライト / preflight check / lint my slides
```
...
```bash
> ノートを整理 / polish notes / add speaker notes to all slides
```
...
```bash
> ファクトチェック / citation check / validate references
```
...
```bash
> テーマ正規化 / normalize theme / replace hex with variables
```
...
```bash
> Remotion best practice / how do I animate in Remotion
```
...
| Graphic Recording | Creation | `"グラレコ"` |
