            ...
            {breadcrumbs.map((item, index) => {
              ...
              return (
                <li key={index} className="flex items-center gap-1.5">
                  {index > 0 && (
                    <span className="select-none text-[var(--ink-soft)]" aria-hidden="true">/</span>
                  )}
                  {isLast || !item.href ? (
                    <span className={isLast ? "font-medium text-[var(--ink)]" : "text-[var(--ink-soft)]"}>{item.label}</span>
                  ) : (
                    <Link href={item.href} className="text-[var(--ink-soft)] transition hover:text-[var(--ink)]">{item.label}</Link>
                  )}
                </li>
              );
            })}
